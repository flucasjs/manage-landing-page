...
export default function Header() {
  ...
  const menuRef = useOutsideClick(setMenuExpanded);
  ...
  return (
    <>
      <header className="max-w-[500px] mx-auto mb-12 flex flex-wrap items-center justify-between px-6 pt-12 lg:mb-24 lg:max-w-full lg:flex-nowrap lg:px-32 lg:pt-16 xl:px-48 2xl:px-[10rem] 3xl:px-[18rem]">
        ...
        <button type="button" className="lg:hidden" onClick={() => setMenuExpanded(v => !v)}>
          {menuExpanded ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="18" className="select-none">
        ...
        <nav
          className={`${menuExpanded ? 'block' : 'hidden lg:block'} mt-10 w-full md:mt-14 lg:mt-0 lg:w-fit`}
          ref={menuRef}
        >
        ...
        <button
          type="button"
          className="hidden whitespace-nowrap rounded-[22px] bg-[#F3603C] px-8 py-[0.8125rem] text-[0.8125rem] font-bold leading-[1.4615] tracking-normal text-[#FCF6F5] shadow-[0_15px_15px_-10px_rgba(255,159,142,1)] hover:bg-[#F98F75] hover:text-[#FCF6F5] lg:block"
        >
          Get Started
        </button>
      </header>
      <div
        className={`fixed inset-0 -z-40 bg-gradient-to-b from-transparent to-black/60 ${menuExpanded ? 'opacity-100' : 'opacity-0'} transition-all  lg:hidden`}
      ></div>
    </>
  );
}
