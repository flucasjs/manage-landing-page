...
import paperPlaneIcon from '@/assets/icons/icon-paper-plane.svg';
import {cn} from '@/util';
...
export default function Footer() {
  ...
  return (
    <footer className="flex flex-col items-center gap-x-4 bg-[#1D1E25] py-[5.6875rem] pb-[1.875rem] pt-[3.1875rem] text-white lg:grid lg:grid-cols-3 lg:grid-rows-2 lg:place-items-center lg:px-[3rem] lg:py-[3.875rem] xl:px-[10.3125rem] xl:py-[4.875rem]">
      <form
        noValidate
        className="mb-[3.375rem] flex gap-x-2 text-[#8D8D8D] lg:col-start-3 lg:row-start-1 lg:mb-0 lg:justify-self-end"
      ...
      >
        <div className="relative">
          <input
            ...
            className={cn(
              'peer rounded-[22px] px-6 py-3 text-[0.8125rem] font-normal leading-[1.4615] -tracking-[0.015em] group-invalid:pointer-events-none',
              !isEmailValid && 'border-red-500'
            )}
          />
          <span
            className={cn('absolute mt-2 hidden text-sm text-red-500', !isEmailValid && 'block')}
          >
            Please insert a valid email
          </span>
          <span
            className={cn('absolute mt-2 hidden text-sm text-green-500', isSubmitted && 'block')}
          >
            Thank you for subscribing!
          </span>
        </div>
        <Button type="submit" className={cn('shadow-none', isSubmitting && 'animate-pulse')}>
          {isSubmitting ? <Image src={paperPlaneIcon} alt="" width={16} height={16} /> : 'Go'}
        </Button>
      </form>
      <ul className="mb-[3.125rem] flex max-w-[16.5rem] flex-wrap gap-x-[6.625rem] gap-y-[0.375rem] text-[0.9375rem] leading-[1.475] -tracking-[0.0165] lg:col-start-2 lg:row-span-2 lg:row-start-1 lg:mb-0 lg:max-w-[24rem] lg:gap-y-[0.8125rem]">
        {['Home', 'Careers', 'Pricing', 'Community', 'Products', 'Privacy Policy', 'About Us'].map(
          item => (
            <li
              key={item}
              className="shrink-0 grow-0 basis-[25%] whitespace-nowrap hover:text-[#F3603C]"
            >
              <Link href="#">{item}</Link>
            </li>
          )
      ...
      <div className="mb-[3.375rem] flex gap-x-[2.125rem] lg:col-start-1 lg:mb-0 lg:self-end lg:justify-self-start">
      ...
      <div className="mb-[3.125rem] lg:col-start-1 lg:row-start-1 lg:mb-0 lg:self-start lg:justify-self-start">
      ...
      <p className="text-[0.8125rem] font-normal leading-[1.4615] -tracking-[0.015em] opacity-50 lg:col-start-3 lg:self-end lg:justify-self-end">
